...
无需构建，运行时由 `tsx` 直接执行 TypeScript 源码。
...
| 写 | `update_campaign` | 全量替换 campaign 及其 lines |
| 写 | `preview_import` | **只解析不写**：返回列映射、缺失必填列、样本行 |
| 写 | `import_performance` | 真正导入业绩（按 line 删旧插新）|
...
2. `preview_import("~/Downloads/nike_july.xlsx")` 核对列映射、确认无缺失必填列
